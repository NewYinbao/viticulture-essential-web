...
func (r *Room) visitorPrompt(p *Player, cardID, stage string, options []string) {
	r.Choices = append(r.Choices, Choice{ID: uid(), PlayerID: p.ID, Kind: "visitor", Options: options, Count: 1, Visitor: &VisitorStep{CardID: cardID, Stage: stage, ActorID: r.Context.ActorID}})
}
