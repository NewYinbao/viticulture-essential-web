...
function hashes(){let files=[];function walk(d){for(const f of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,f.name);if(f.isDirectory()){if(f.name==='web'||d!==root)walk(p)}else if(d!==root||/\.(go|exe)$/.test(f.name)||f.name==='ee_cards.json'||f.name==='go.mod')files.push(p)}}walk(root);return Object.fromEntries(files.sort().map(p=>[path.relative(root,p),crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex')]))}
const delay=ms=>new Promise(r=>setTimeout(r,ms));
(async()=>{let browser,server;const tmp=fs.mkdtempSync('/tmp/viticulture-stages-');m.temporaryDirectory=tmp;m.sourceHashesBefore=hashes();persist();
try{
execFileSync('/opt/viticulture-toolchain/go/bin/go',['build','-o',path.join(tmp,'ee-server'),'.'],{cwd:root,env:{...process.env,GOOS:'linux',GOARCH:'amd64'},timeout:120000});
...
server=spawn(path.join(tmp,'ee-server'),['-addr','127.0.0.1:'+port,'-data',m.dataDirectory],{cwd:root,stdio:['ignore','pipe','pipe']});m.serverPID=server.pid;let logs='';server.stdout.on('data',d=>logs+=d);server.stderr.on('data',d=>logs+=d);
...
fs.mkdirSync(path.join(out,'regression'),{recursive:true});execFileSync('/opt/splendor-runtime/bin/node',['-e',fs.readFileSync(path.join(root,'ee-browser-test.cjs'),'utf8').replace('http://127.0.0.1:13013',base).replace("out='/mnt/c/Users/admin/Games/Viticulture/essential-web/'","out="+JSON.stringify(path.join(out,'regression')+'/'))],{cwd:root,timeout:120000});m.existingBrowserRegression='passed';await pages[0].goto(base);await capture('entry','01-entry.png',pages[0]);await pages[0].locator('#nickname').fill('山丘庄主');await pages[0].locator('#create-room').click();await pages[0].locator('#code-label').waitFor({state:'visible'});let s=await snap(pages[0]);m.roomCode=s.code;
...
async function testDiscard(p,s,pages,ids,snap,sync,capture,act,m,base){
 ...
 const forged=await other.evaluate(async a=>{const r=await fetch('/api/action',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+localStorage.getItem('vineyard-ee-token')},body:JSON.stringify(a)});return r.status},{type:'choose',revision:s.revision,choiceId:s.pendingChoice.id,cardIds:s.hand.slice(0,count).map(c=>c.id)});
