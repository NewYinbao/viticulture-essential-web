...
await pages[0].goto(base);await capture('entry','01-entry.png',pages[0]);await pages[0].locator('#nickname').fill('山丘庄主');await pages[0].locator('#player-password').fill('test-password-123');await pages[0].locator('#create-room').click();await pages[0].locator('#code-label').waitFor({state:'visible'});let s=await snap(pages[0]);m.roomCode=s.code;
