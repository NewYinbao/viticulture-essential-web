...
fs.mkdirSync(path.join(out,'regression'),{recursive:true});execFileSync(process.execPath,[path.join(__dirname,'ee-browser-test.cjs')],{cwd:root,env:{...process.env,E2E_BASE_URL:base,E2E_OUT_DIR:path.join(out,'regression')},timeout:120000});m.existingBrowserRegression='passed';await pages[0].goto(base);await capture('entry','01-entry.png',pages[0]);await pages[0].locator('#nickname').fill('山丘庄主');await pages[0].locator('#create-room').click();await pages[0].locator('#code-label').waitFor({state:'visible'});let s=await snap(pages[0]);m.roomCode=s.code;
