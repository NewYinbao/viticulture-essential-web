...
func TestVisitorPermissionRollbackReplayAndPersistence(t *testing.T) {
	...
	send := func(token string, a Action) int { return request(app.routes(), "POST", "/api/action", token, a).Code }
	if code := send("0", Action{Type: "place", Space: "winter_visitor", CardID: "winter-38", Slot: 2}); code != 200 {
		t.Fatal(code)
	}
	r = app.Store.Rooms[r.Code]
	c := r.Choices[0]
	act := Action{Type: "choose", ChoiceID: c.ID, Option: "resolve", Revision: r.Revision}
	before, _ := json.Marshal(r)
	if send("1", act) != 400 {
	...
	r = app.Store.Rooms[r.Code]
	c = r.Choices[0]
	act = Action{Type: "choose", ChoiceID: c.ID, Option: "train", Revision: r.Revision}
	if send("0", act) != 200 {
		t.Fatal("actor reply")
	}
	app, e = loadApp(dir)
	...
	c = r.Choices[0]
	if send("2", Action{Type: "choose", ChoiceID: c.ID, Option: "skip", Revision: r.Revision}) != 200 {
		t.Fatal("skip")
	}
	app, e = loadApp(dir)
	if e != nil {
...
func roundTripVisitor(t *testing.T, r *Room) *Room {
	t.Helper()
	dir := t.TempDir()
	app, e := loadApp(dir)
	if e != nil {
