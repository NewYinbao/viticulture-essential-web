...
func eeRoom(n int) *Room {
	...
	for i := 0; i < n; i++ {
		r.Players = append(r.Players, NewPlayer("p"))
	}
...
func TestEEChoiceSaveResume(t *testing.T) {
	...
	c := q.Choices[0]
	nonOwner := q.Players[(q.SpringLeader+1)%len(q.Players)].ID
	if q.Apply(nonOwner, Action{Type: "choose", ChoiceID: c.ID, Option: "gift"}) == nil {
