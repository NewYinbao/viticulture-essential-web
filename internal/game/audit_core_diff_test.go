...
// Regression specifications for differences identified in the original rule audit.
func TestAuditDiffPlantMustNotOfferUproot(t *testing.T) {
	p, q := NewPlayer("a"), NewPlayer("b")
	p.Wake = 1
	q.Wake = 2
	p.Fields[0].Vines = []Card{{ID: "v", Type: "vine", Red: 1}}
	r := &Room{Players: []*Player{p, q}, Phase: "summer", Year: 1, TurnID: p.ID, Spaces: NewSpaces(2)}
...
func TestAuditDiffYokeReopensAfterProducer(t *testing.T) {
	...
	p.YokeUsed = true
	r := &Room{Players: []*Player{p}, Spaces: NewSpaces(2), Context: &ActionContext{Space: "summer_visitor"}}
...
func TestAuditDiffProducerMayRetrieveEarlierWorkerOnSameAction(t *testing.T) {
	p := NewPlayer("a")
	p.Coins = 2
	r := &Room{Players: []*Player{p}, Spaces: NewSpaces(3), Context: &ActionContext{Space: "summer_visitor"}}
	for i := range r.Spaces {
...
func TestAuditDiffBaseWorkerAndEndgameControls(t *testing.T) {
	...
	p.LargeWorker = false
	p.Trained = 3
	r := &Room{Players: []*Player{p, q}, Year: 1, Spaces: NewSpaces(2)}
	r.finishYear()
